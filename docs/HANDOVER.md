...
| 2026-09-15 | 生产部署准备 | 目标：新人 clone 后一条命令跑起来。改动：①前后端 Dockerfile 改成多阶段构建，`dev` / `prod` 两个 target（生产镜像不再挂载源码、以非 root 用户运行、后端多 worker、前端用 Next.js standalone 输出 + `node server.js`），开发 compose 显式指定 `target: dev` 保持原行为不变；②新增 `docker-compose.prod.yml`：只有 API 与前端对外发布端口，Postgres/Redis/Chroma/MinIO 留在内部网络，新增一次性 `migrate` 服务在 API 与 worker 之前执行 `alembic upgrade head`，全部服务带健康检查与 `restart: unless-stopped`，数据落命名卷；③新增 `scripts/deploy.ps1` / `deploy.sh`（缺 `.env` 时自动从模板生成、构建启动、等待健康检查、打印访问地址）与 `start.sh` / `stop.sh`；④`.env.example` 补充 `BACKEND_PORT` / `FRONTEND_PORT` / `UVICORN_WORKERS` 与上线前必须修改项说明；⑤新增 `docs/DEPLOYMENT.md`（架构说明、变量参考表、上线检查清单、运维与排错）并在 README 增加生产部署章节。踩坑：`# syntax=docker/dockerfile:1` 会去 Docker Hub 拉构建器镜像，本机代理下失败，已移除；构建上下文缺少 `.dockerignore`，宿主机的 Windows `node_modules` 覆盖了容器内依赖导致 `pnpm build` 找不到 next，已为前后端各加 `.dockerignore`（排除 node_modules/.next/缓存/本地数据）。实测：生产镜像构建成功，用备用端口（18000/13000）独立项目名启动，migrate 自动跑到 `476776c615f9`、创建 7 张表，后端 `/health` 返回 ok，前端首页 HTTP 200 |
