...
1. 把 `.env` 里的 `NEXT_PUBLIC_API_BASE_URL` 改成这台机器的局域网地址，例如 `http://192.168.1.10:8000`；
2. 重新构建前端镜像：`docker compose -f docker-compose.prod.yml up -d --build`；
3. 在系统防火墙里放行 3000 与 8000 端口。

> ⚠️ 注意：接口目前**没有鉴权**，只适合本机或可信的内网使用。不要把它直接暴露到公网——
> 任何知道文档 ID 的人都能读取文档、知识树与问答结果。
